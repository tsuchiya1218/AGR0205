...
<body>
    ...
    <main>
        ...
        $cid = $_GET["data"] == null ? "未入力" : sprintf("%s", htmlspecialchars(($_GET["data"]), ENT_NOQUOTES, 'UTF-8'));
        $witer = $_GET["witer"] == null ? "未入力" : sprintf("%s", htmlspecialchars(($_GET["witer"]), ENT_NOQUOTES, 'UTF-8'));
        $publisher = $_GET["publisher"] == null ? "未入力"  : sprintf("%s", htmlspecialchars(($_GET["publisher"]), ENT_NOQUOTES, 'UTF-8'));
        $tagName = $_GET["tagName"] == null ? "未入力"  : sprintf("%s", htmlspecialchars(($_GET["tagName"]), ENT_NOQUOTES, 'UTF-8'));
        $composer = $_GET["composer"] == null ? "未入力"  : sprintf("%s", htmlspecialchars(($_GET["composer"]), ENT_NOQUOTES, 'UTF-8'));
        $direct = $_GET["direct"] == null ? "未入力"  : sprintf("%s", htmlspecialchars(($_GET["direct"]), ENT_NOQUOTES, 'UTF-8'));
        ...
        <table border="1">
            ...
            if ($cid == null ) {
                ...
                // SQL文を実行
                $stmt = $dbh->prepare($sql);
                $stmt->execute(array(('%' . $cid . '%'), ('%' . $witer . '%'), ('%' . $publisher . '%'), ('%' . $composer . '%'), ('%' . $direct . '%')));
